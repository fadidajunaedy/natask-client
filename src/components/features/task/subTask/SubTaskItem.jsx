        ...
        <li className="border-t first:border-t-0 border-base-300 py-2 flex justify-between items-center gap-4">
            ...
            <div className="flex items-center gap-2">
                <div className="dropdown dropdown-left dropdown-end z-[2] ml-auto">
                    ...
                    >
                        <Button level="none" onClick={() => handleChangeStatus("toDo")} loading={loading}>
                            <TbCircle /> To Do
                        </Button>
                        <Button  level="info" onClick={() => handleChangeStatus("inProgress")} loading={loading}>
                            <TbProgressCheck /> In Progress
                        </Button>
                        <Button  level="warning" onClick={() => handleChangeStatus("inReview")} loading={loading}>
                            <TbEditCircle /> {viewMode === "DASHBOARD" ? "In Review" : "Ask for Review"}
                        </Button>
                        {viewMode === "DASHBOARD" && 
                        <Button  level="success" onClick={() => handleChangeStatus("done")} loading={loading}>
                            <TbCircleCheck /> Done
                        </Button>
                        }
                ...
                <Button 
                    level="error"
                    square
                    onClick={() => dispatch(openModal({ key: "SUBTASK", type: "DELETE", _id: subTask._id}))}
                >
                    <FiTrash />
                </Button>
                }
